...
class PlayerWithNaiveStrategy(Agent):
    def __init__(self, unique_id, model, pm, delay, probability, decrease_probability):
        ...
        self.decrease_probability = decrease_probability

    def step(self):

        # Aggregated signal
        self.aggregated_signal = self.model.public_signal_value + self.private_signal_value - self.signal_decrease_value
...
class PlayerWithLastMinute(Agent):
    def __init__(self, unique_id, model, pm, time_reveal_delta, time_estimate, delay, probability, decrease_probability) :
        super().__init__(unique_id, model)
        self.pm = pm
        self.bid = 0
...
class PlayerWithBluffStrategy(Agent):
    def __init__(self, unique_id, model, pm, time_reveal_delta, time_estimate, bluff_value, delay, probability, decrease_probability):
        super().__init__(unique_id, model)
        self.pm = pm
        self.bid = 0
        self.time_reveal_delta = time_reveal_delta
...
def get_signal_derease_max(model):
    return model.signal_decrease_max


class Auction(Model):
    def __init__(self, N, A, L, S, B, rate_public_mean, rate_public_sd, rate_private_mean, rate_private_sd, T_mean, T_sd, delay, rate_decrease) :
        self.num_naive = N
        self.num_adapt = A
        ...
        self.datacollector = mesa.DataCollector(
            model_reporters={
                ...
                "Signal Decrease Max": get_signal_derease_max
            },
    ...
    def step(self):
        ...
        if self.schedule.time > int(self.cancel_time * 100):
            new_signal_decrease = poisson.rvs(mu = self.decrease_rate*1200/50)
            self.signal_decrease += new_signal_decrease

            for _ in range (new_signal_decrease):
                signal_decrease_value = 0.002
                self.signal_decrease_max += signal_decrease_value
                for agent in self.schedule.agents:
                    if random.random() < agent.decrease_probability:
                        agent.signal_decrease_value += signal_decrease_value
...
print(f"Winner True Profit: {model.winner_trueprofit}")
...
# plt.grid(True)
# plt.xticks(np.arange(0, int(model.T*100), 10))  # Set x-axis to be 0, 10, 20, 30... etc
